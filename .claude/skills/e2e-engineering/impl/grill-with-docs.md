...
Runs ONCE at Implementation entry, BEFORE to-issues. Reconciles PRD + (brownfield) codebase-map "existing language" (section 4) against the CONTEXT.md glossary. NOT per-slice — slices inherit shared language via CONTEXT + constitution. Per-slice gap-finding is the [slice gap-check](./tdd.md)'s job. (The standalone grill-with-docs skill at repo root covers the general technique; this is its fixed placement in e2e-engineering.)
