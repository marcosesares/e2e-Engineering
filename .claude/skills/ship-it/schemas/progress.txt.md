...
Append-only learnings log for one Task. **caveman:ultra** style (max compression, full technical substance). Sole writer = orchestrator, at fan-in. Append-only WITHIN a task; reset (overwrite empty) when a new task begins. Lives at task root (e.g. `.ship-it/progress.txt`).
