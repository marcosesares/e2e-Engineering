---
name: ship-it
description: Master engineering orchestrator — drives a Task from idea to passing E2E across pre-implementation, implementation, and post-implementation phases. Detects phase and task type, sequences sub-skills, runs the vertical-slice TDD loop with parallel subagents, enforces five hard gates, and checkpoints at 65% context. Handles greenfield, feature, bugfix, and refactor on new or existing codebases, plus a one-time `adopt` mode for onboarding an in-progress project. Use when the user says "ship it", "build this end to end", "run the full flow", "/ship-it", or wants the complete engineering pipeline rather than a single isolated step.
---
...
State lives under `.ship-it/` at repo root: `prd.json`, `progress.txt`, `codebase-map.md` (brownfield), `research.md` (if research ran), `test-cases/*.md`, handoff docs. Schemas: [prd.json](./schemas/prd.json.md), [progress.txt](./schemas/progress.txt.md), [codebase-map](./schemas/codebase-map.md).
...
- User invoked `/ship-it adopt` → run [adopt](./adopt.md). One-time onboarding, not the per-task flow. Stop here.
...
2. If a handoff doc exists (`.ship-it/handoff-*.md`), this is a fresh-session resume → run [phase-transition](./cross/phase-transition.md) bootstrap FIRST (read handoff → prd.json → progress.txt → invoke suggested skill).
