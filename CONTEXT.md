...
**Hard gate**: Non-negotiable checkpoint an agent cannot pass (agent-internal ones are non-overridable; only gate 1 needs explicit human consent). The four (ADR 0024 retired gate 4): (1) PRD approved → implementation, (2) TDD red phase — failing test before production code; layers = unit (Vitest) + **API/integration (Playwright `request`, red-green per implemented API call)**, (3) debug escalation (after 3 failed fixes, stop + escalate), (5) verification-before-completion = full unit+API suite green + AC-checklist-vs-code, in self-review. Each surfaces as a red-flags line in its sub-skill. Concept from superpowers. **Gate 4 (E2E green → post-impl) RETIRED — ADR 0024 (Fork Y):** UI E2E not automated, verified manually in human-QA. Label 5 kept (not renumbered).
...
**Verification-before-completion** (hard gate 5): Final impl-phase check, executed inside self-review. **Rescoped by ADR 0024 (Fork Y):** = full automated suite re-run (unit + API/integration, all tests not just changed slices) green **+** AC-checklist against code (every PRD acceptance criterion maps to implementation). **No live-UI exercise** — `/run` + `/verify` dropped (agent live-UI verification too costly; UI is verified by the human-QA Manual walk post-impl). Blocks `done` on a red suite or an unmet AC. Passing = implementation done → `pending-qa` → human-QA.
_Avoid_: invoking /run + /verify or opening the app for UI checks here (removed — Fork Y); treating UI ACs as code-verifiable beyond "a test/handler exists" (behavioral UI proof is the human walk's job)
