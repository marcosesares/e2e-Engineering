...
> - **Expert agent** (NEW): role-prompted reviewer sub-agent (`backend-architect`, `dba`, `frontend-reviewer`, `test-reviewer`). Claude Code uses generated `.claude/agents/` wrappers; Codex uses standard worker agents with the canonical expert spec injected into the prompt. A second fan-out wave reviews each green slice before merge (findings Critical/Important/Minor, bounce cap 3), and advises the PRD in pre-impl planning.
...
**Hard gate**: Non-negotiable checkpoint an agent cannot pass (agent-internal ones are non-overridable; only gate 1 needs explicit human consent). The four (ADR 0024 retired gate 4): (1) PRD approved → implementation, (2) TDD red phase — failing test before production code; layers = unit (Vitest) + **API/integration (Playwright `request`, red-green per implemented API call)**; mocked unit tests do NOT satisfy Gate 2 for API endpoint slices, (3) debug escalation (after 3 failed fixes, stop + escalate), (5) verification-before-completion = full unit+API suite run + AC-checklist-vs-code, in self-review; **failures → pending-qa + qa-signoff Gate 5 Failures section, NOT blocked (ADR 0025)**. Each surfaces as a red-flags line in its sub-skill. Concept from superpowers. **Gate 4 (E2E green → post-impl) RETIRED — ADR 0024 (Fork Y):** UI E2E not automated, verified manually in human-QA. Label 5 kept (not renumbered).
...
**Verification-before-completion** (hard gate 5): Final impl-phase check, executed inside self-review. **Rescoped by ADR 0024 (Fork Y):** = full automated suite re-run (unit + API/integration, all tests not just changed slices) green **+** AC-checklist against code (every PRD acceptance criterion maps to implementation). **No live-UI exercise** — `/run` + `/verify` dropped. **ADR 0025:** suite red or unmapped AC does NOT block — failures recorded in `qa-signoff.md ## Gate 5 Failures`, Task proceeds to `pending-qa`. Human routes each failure to a repair Task at QA sign-off. `blocked` reserved for Gate 3 exhausted stories.
_Avoid_: invoking /run + /verify or opening the app for UI checks here (removed — Fork Y); treating UI ACs as code-verifiable beyond "a test/handler exists" (behavioral UI proof is the human walk's job); marking Task `blocked` on red Gate 5 suite (ADR 0025)
...
**E2E gate** _[RETIRED — ADR 0024 (Fork Y)]_: Was the condition requiring the E2E suite green before the impl loop exits (old gate 4). Gone: UI is not automated. The executable net is now unit + API/integration (gate 2 red-green) + full-suite-green at gate 5; UI correctness is the human-QA Manual walk.
...
**E2E test** _[scope narrowed — ADR 0024 (Fork Y)]_: Executable automation of a Test case. **API/integration only** — Playwright `request`, authored red-green IN-SLICE per implemented API call (gate 2), per the `standards/api-testing.md` baseline (mechanic **M1**: run against the docker-compose stack; project's own API-test conventions in [[ARCHITECTURE.md]] §4.1 override the baseline). **UI E2E is NOT automated** (no Playwright browser/POM in this flow): UI Test cases are disposition Manual → human-QA walk. ADR 0010's feature-in-slice / regression-final-pass split now applies to API tests only; the UI regression pass is dropped. On failure, the slice reads the Playwright report and loops fixing under gate 3 (3-strike). Traceability: each E2E test maps back to a Test case ID.

**Disposition** (lite): A Test case is **Automated** (covered by an E2E test) or **Manual** (walked by a human). The Manual set is the post-implementation human-QA script. Borrowed from qa_skills dispositions, stripped of ADO coverage fields. **ADR 0024 (Fork Y):** unit + API/integration cases → Automated; **all UI cases → Manual** (no UI automation path).
...
**map-codebase** (sub-skill): Conditional pre-impl step — fires only on brownfield (task targets existing code). Produces `codebase-map.md`, SCOPED to *this* change (sprint-lifetime, can rot like [[Research]]), with 5 sections: (1) blast-radius modules, (2) seams/adapters (where tests attach), (3) local impact list, (4) existing language — terms only, fed to grill-with-docs (durable structure/ownership/naming live in [[ARCHITECTURE.md]], NOT §4), (5) refactor candidates. Also reconciles the code against [[ARCHITECTURE.md]] and proposes additions/corrections for human review. NOT a global C4/ERD/NxN matrix. **Gate 1 enforces existence:** brownfield task missing `codebase-map.md` at gate 1 → `/e2e-engineering` stalls: "Pre-impl incomplete — run map-codebase first." Flight Step 2 stalls with `<e2e-stall reason="codebase-map-missing">` if absent — no cold-read fallback. See ADR 0009.
_Avoid_: full C4/ERD/spec-impact matrices (too heavy, rots per slice); the global reverse-engineering artifacts the prototypes were studied with; cold-reading source files in flight when map missing (stall instead)
...
**Codex branch-visible integration**: Token-cheap Codex slice integration mode. Each implementation worker is instructed to create and commit to `slice/<story-id>`, then return the branch name plus [[slice result manifest]]. The orchestrator sees that branch in its git context, runs diff/tests/review locally, and owns the merge into the Task branch. Step 0 verifies this with a disposable branch commit before any real slice work. If the branch or commit is not visible, e2e-flight stalls with `<e2e-stall reason="worker-changes-unavailable" />`; if the worker mutates the orchestrator checkout, e2e-flight uses [[Codex serial branch mode]].
_Avoid_: returning full code patches through chat for normal slices (token-expensive, poor audit trail); assuming branch visibility without probing; letting workers merge into the Task branch

**Codex serial branch mode**: Explicit degraded Codex implementation mode used when workers can commit visible branches but cannot preserve orchestrator checkout isolation. The orchestrator dispatches one slice at a time, switches to the slice branch before spawning the worker, waits for the manifest, validates, switches back to the Task branch, then merges.
_Avoid_: calling this parallel fan-out; running more than one implementation worker at a time in this mode

**Slice result manifest**: Evidence-pointer-first structured result every fan-out worker returns to the orchestrator at fan-in. Fields: `sliceId`, `status`, one-line `summary`, `testsPassed`, `branch`, `evidencePaths[]`, `findings[]`. Orchestrator validates branch + evidence paths before accepting green — missing evidence = bounce. [[Sole writer]] then writes authoritative `status` to `prd.json` (status authority lives there, not in sidecar) and persists the manifest to [[evidence sidecar]] at `manifests/<story-id>/slice-result.json`.
_Avoid_: raw test logs, full diffs, or long implementation narratives in worker final messages
...
**Review bundle**: Manifest-first sidecar the orchestrator writes before expert review. Contains branch names, base/head commits, reviewer list, changed files, diff stat, test outcomes, and paths to full evidence; reviewers pull scoped hunks/logs themselves.
_Avoid_: orchestrator-authored full diff summaries; pasting raw logs into every reviewer prompt

**Evidence sidecar**: Durable gate evidence record living at `.e2e-engineering/manifests/<story-id>/` — `slice-result.json` ([[slice result manifest]]), `review-bundle.json` ([[review bundle]]), `review-result.json` ([[review manifest]]), and `verification-result.json`. `prd.json` holds lightweight pointers (`resultManifestPath`, `reviewManifestPath`) + authoritative status; sidecars hold evidence metadata, pointers, and findings. Cross-runtime — applies to both Claude Code and Codex variants. Status authority always in `prd.json`; [[sole writer]] reconciles sidecar status at fan-in and writes the authoritative story status there.
...
**Expert agent**: Independent, fresh-context reviewer sub-agent spawned AFTER the impl wave, BEFORE merge. Reviewer independence is the core value — reviewers must not share the implementation thread's context. Input is artifact-driven: each reviewer receives a [[review bundle]] and its [[canonical expert spec]]. Reviewer is read-only by contract — never edits files. Returns a [[review manifest]]. Fan-out by expertise area (not per slice) catches cross-slice integration issues.
...
_Avoid_: treating these labels as Codex `spawn_agent.agent_type` values

**Spawn agent type**: Runtime tool role value used to create a sub-agent, such as Codex `worker`, `explorer`, or `default`.
_Avoid_: "reviewer role" when referring to the tool-level role field

**Canonical expert spec**: Runtime-neutral expert review definition living at `skills/e2e-engineering/agents/*.md`. Contains review rubric, expertise scope, output contract — pure prose, no runtime primitives. Source of truth for [[expert agent]] behavior. Claude Code wrappers (`.claude/agents/*.md`) are generated from this source via [[agent wrapper generation]]. Codex injects this spec directly into standard `worker` prompts because generated custom TOML roles are not guaranteed to surface as spawnable runtime agent types. Expertise is canonical; runtime policy (model, sandbox_mode, mcp_servers, invocation format) stays adapter-specific.
_Avoid_: putting model selection or sandbox policy into canonical spec (adapter concerns); relying on generated Codex TOML roles as load-bearing dispatch targets

**Agent wrapper generation**: Script (`skills/e2e-engineering/scripts/generate-agent-wrappers.ps1`) that reads `skills/e2e-engineering/agents/*.md` + `agents.manifest.json` and emits self-contained Claude Code adapter files: `.claude/agents/<role>.md`. Generated files inline the canonical expert instructions rather than path-referencing them. Checked into source and regenerated when canonical specs change — never edited by hand. Codex has no generated wrapper dependency; the orchestrator injects the same canonical specs into `worker` prompts.

**Expert-review wave**: Second [[fan-out / fan-in]] wave in e2e-flight, after impl wave, before merge. Orchestrator spawns [[expert agent]]s by expertise area, each receiving review artifacts (not worktrees). Before dispatch, orchestrator reads existing test method signatures (not bodies) for files touched by the slice and injects as `existingTests[]` — prevents false Critical findings from missing coverage already present outside the diff. Reviewers must cite a specific line/test proving a coverage gap before assigning Critical; un-evidenced Criticals rejected without bounce. Findings dispatched via [[Three-tier bounce]]. Parallel reviewer dispatch preferred; Codex may use bounded batches when runtime slots are constrained. Fan-in collects [[review manifest]]s, deduplicates, enforces severity gates.
...
**Slice dispatch**: Orchestrator delegates each vertical slice to a fresh subagent that runs red-green-refactor for that slice, returns a [[slice result manifest]], which the orchestrator validates before marking the story done. The worker commits slice changes, but the orchestrator owns merge into the [[Task]] branch. Keeps orchestrator context lean (sees manifests, not raw TDD churn). Borrowed from superpowers `subagent-driven-development`; rejected inline TDD (`executing-plans`). Runtime-specific: Claude Code uses `EnterWorktree` + `Agent`; Codex uses [[Codex fan-out]] manifest-driven spawn.
_Avoid_: "inline TDD" (orchestrator does not write slice code itself)

**Forcing mechanism**: Bootstrap guard that proves worker dispatch works before any slice work begins. Fail-closed — any failure → stall + EXIT; inline slice-impl = hard STOP. Protocol: (1) static capability hint — use tool-discovery primitive if available in runtime, not sufficient alone; (2) live no-op probe — spawn trivial worker, wait for `{"status":"ok","capability":"fanout-probe"}`, close worker; short timeout; (3) Codex worker-change probe — disposable branch commit must be visible to the orchestrator before slice work; unchanged orchestrator branch/HEAD enables parallel Codex mode, changed branch/HEAD enables [[Codex serial branch mode]]. Claude Code variant: `ToolSearch` loads `Agent`+`EnterWorktree`; Codex variant: static hint + live spawn probe + worker-change probe. The structural token fix — guarantees worker dispatch fires rather than silently falling back to inline work (the 22.3M-token blowup cause). Skill trigger should declare sub-agent usage explicitly so runtimes requiring user authorization can prompt before Step 0 runs.
_Avoid_: treating static capability check as sufficient (proves configuration, not runtime); proceeding past Step 0 when branch visibility fails; natural-language fallback as the backup path; full-patch chat fallback for normal Codex slices

**Dual-runtime skill tree**: Directory layout supporting Claude Code and Codex runtimes from one canonical source. Three-zone structure: (1) `skills/` (repo root) — runtime-neutral shared sub-skills, schemas, constitution, [[canonical expert spec]]s; (2) `.claude/skills/` — Claude Code entry-point `SKILL.md` files only (runtime-coupled); (3) `.agents/skills/` — Codex entry-point `SKILL.md` files only (runtime-coupled). Expert agent Claude wrappers are generated via [[agent wrapper generation]] into `.claude/agents/`; Codex expert roles are prompt-injected worker agents. Rule: if a file contains no runtime primitives, it lives in `skills/`; if it references `EnterWorktree`/`Agent`/`spawn_agent`/etc., it lives in the runtime-specific entry point.
...
**Shared skills root**: Bootstrap-resolved path to the runtime-neutral shared e2e-engineering skill tree, normally `skills/e2e-engineering` at repo root.
_Avoid_: probing `.agents/skills/...` or `.claude/skills/...` for shared sub-skills, schemas, constitution, or canonical expert specs during execution

**Dual-runtime install**: Distribution contract for the [[dual-runtime skill tree]]. Claude installs `skills/` + `.claude/skills/` + `.claude/agents/`. Codex installs `skills/` + `.agents/skills/` + `AGENTS.md`. Cursor/OpenCode use the Codex shape, with Cursor also receiving `.cursor/rules`. `AGENTS.md` is not the flow body; it is only the router into `.agents/skills/`.
_Avoid_: AGENTS.md-only Codex installs (router points to missing skills); installing thin runtime wrappers without the shared `skills/` tree they link to

**Known-renames cleanup**: Installer cleanup rule for deprecated generated files left by older installs. The installer warns when a known old path exists and deletes it only with `--force`. Current known renames: `.claude/agents/ui-designer.md` -> `.claude/agents/frontend-reviewer.md`; `.claude/agents/senior-qa.md` -> `.claude/agents/test-reviewer.md`.
_Avoid_: silent deletion; broad glob cleanup; keeping compatibility shims for renamed expert roles
...
**/e2e-flight**: Headless implementation worker, sibling to [[/e2e-engineering]] (ADR 0022). Implements exactly ONE Task from the [[Task queue]] per invocation, then exits — no driver loop, no context monitoring; re-invoke for the next Task. WITHIN the spawn it IS the orchestrator: Step 0 forces fan-out (`ToolSearch`-loads `Agent`+`EnterWorktree`; inline slice-impl = hard STOP), then per slice → impl sub-agent wave → [[Expert agent]] review wave (in worktree, before merge) → merge → record. After the DAG drains: e2e-QA stub (gates 4/5 stubbed) → self-review → write qa-signoff.md → exit. **Branch model:** orchestrator works on `task/<id>` branch throughout; sub-agents in isolated worktrees; master receives exactly two targeted `queue.json` commits — `todo→in-progress` before branching (task lock, Step 1) and `in-progress→pending-qa` after self-review pass (Step 5.1). Flight stalls at Step 1 if master has uncommitted changes.
_Avoid_: "the orchestrator" (that's [[/e2e-engineering]]'s interactive role), "external driver" / "detached window" (flight runs in the current session, no loop); git stash; touching master mid-flight beyond the two targeted commits
...
**qa-signoff.md** (`tasks/<id>/qa-signoff.md`): Per-Task human checklist written by flight when it defers human-QA. Holds manual test cases to walk, auto-verified ACs to eyeball, pending amendments to promote/drop, [[Gate 5 failure]]s (if any), and a [[QA finding]]s section. The audit record; [[Task queue]] holds the actionable state.
...
**Gate 5 failure**: A failing automated test or unmapped AC surfaced by [[verification-before-completion]] and recorded in `qa-signoff.md ## Gate 5 Failures` by flight. Does NOT block or mark the Task `blocked` — Task proceeds to `pending-qa`. During [[QA sign-off session]], human routes each through `triage` into a repair Task (real regression → bugfix Task; flaky/pre-existing → drop). Built Task still goes `done`. ADR 0025.
...
- Implementation workers commit slice branches; reviewers validate read-only; bounce fix workers commit fixes; the orchestrator alone merges into the **Task** branch.
- **Codex serial branch mode** keeps worker token isolation but removes intra-Task implementation parallelism.
...
- A Codex **Expert agent** uses `worker` as the **Spawn agent type** and injects a **Reviewer prompt role** via the prompt.
- `/e2e-flight` resolves **Shared skills root** once at bootstrap and reuses it for every shared file read.
...
- "backend-architect/test-reviewer/etc." — resolved: in Codex these are **Reviewer prompt roles**, not **Spawn agent types**; use `worker` as the tool role and inject the canonical expert spec.
- "branch-visible" — sufficient for **Codex serial branch mode**, but not for parallel Codex mode; parallel Codex workers must also prove checkout isolation by leaving the orchestrator branch/HEAD unchanged.
- "entry point" vs "shared skill" — resolved: `.agents/skills/...` and `.claude/skills/...` route runtime-specific entry points only; shared content is under **Shared skills root**.
