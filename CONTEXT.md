...
**Expert-review wave**: Second [[fan-out / fan-in]] wave in e2e-flight, after impl wave, before merge. Orchestrator spawns [[expert agent]]s by expertise area, each receiving review artifacts (not worktrees). Parallel reviewer dispatch is preferred; Codex may use bounded reviewer batches after closing completed/errored agents when runtime slots are constrained. Fan-in collects [[review manifest]]s, deduplicates, enforces severity gates.
_Avoid_: inline orchestrator review as a substitute for an expert agent; skipping `test-reviewer`
