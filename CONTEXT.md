...
**Codex branch-visible integration**: Token-cheap Codex slice integration mode. Each implementation worker is instructed to create and commit to `slice/<story-id>`, then return the branch name plus [[slice result manifest]]. The orchestrator sees that branch in its git context, runs diff/tests/review locally, and merges it. Step 0 verifies this with a disposable branch commit before any real slice work. If the branch or commit is not visible, e2e-flight stalls with `<e2e-stall reason="worker-changes-unavailable" />`.
_Avoid_: returning full code patches through chat for normal slices (token-expensive, poor audit trail); assuming branch visibility without probing
...
**Forcing mechanism**: Bootstrap guard that proves fan-out works before any slice work begins. Fail-closed — any failure → stall + EXIT; inline slice-impl = hard STOP. Protocol: (1) static capability hint — use tool-discovery primitive if available in runtime, not sufficient alone; (2) live no-op probe — spawn trivial worker, wait for `{"status":"ok","capability":"fanout-probe"}`, close worker; short timeout; (3) Codex branch-visibility probe — disposable branch commit must be visible to the orchestrator before slice work. Claude Code variant: `ToolSearch` loads `Agent`+`EnterWorktree`; Codex variant: static hint + live spawn probe + branch visibility probe. The structural token fix — guarantees fan-out fires rather than silently falling back to inline work (the 22.3M-token blowup cause). Skill trigger should declare sub-agent usage explicitly so runtimes requiring user authorization can prompt before Step 0 runs.
_Avoid_: treating static capability check as sufficient (proves configuration, not runtime); proceeding past Step 0 when probe fails; natural-language fallback as the backup path; full-patch chat fallback for normal Codex slices
