...
- (filled DURING the QA session by the human)
...
- **Flight fills** (headless): Manual test-case list (from prd.json testCases with disposition Manual), AC list with auto-verified ticked, pending amendments staged from progress.txt.
...
Human-QA cannot run headless. Flight runs everything automatable (gates 4, automated half of 5, review), parks human judgment here, sets Task `pending-qa`, and continues to the next Task. All checklists cleared in one batched session at the end — mirrors pattern-promotion batching (ADR 0014 lineage). See ADR 0018.
